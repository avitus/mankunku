...
- **Trick** and its parameters (Tricks only) — Enclosures use an interactive Phrase canvas: choose the target chord tone, approach pattern, note count, and landing beat directly on the graphic. **Practice over** offers major/minor/dominant single chords and long ii–V–I in major or minor; tap a chord cell to preview its arrival. Single-chord practice requires an unlocked mastery variant; long progressions accept any valid combination and save their progress separately. Triad pairs retain parameter pads and a *Next unlock* hint. See [Practicing Tricks](./tricks.md).
