...
There's **no sheet music during a session** — you read the changes, not the line — with one exception: a key that's beating you. Once your rolling score in a key drops under 75%, that key's row in the scrolling stack turns into a lead sheet: the line engraved with its changes above the staff, in your instrument's written pitch, the note the band is at lit as it goes, the beat dots beneath, and a caption — *Sheet music while D is under 75%* — saying why. It stays for as long as the key does; bring it back over the line and the row goes back to chord blocks on its own. A key you've never played is always by ear first — the sheet only ever answers a miss. There's nothing to switch on or off. If you'd rather study the line before you start, it's still on the lick's page.
