...
When a key does unlock, the tempo **drops 10%** instead of taking that session's bump. An unlock only ever follows a strong session, so without the drop the brand-new key would arrive *faster* than the tempo that earned it — instead it arrives with headroom, and you win the speed back as you consolidate the bigger rotation. On the lick's progress chart this reads as a sawtooth: climb, unlock, dip, climb again.
