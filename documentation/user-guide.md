...
- **The sheet music is conditional too.** While the key you're learning — the most recently unlocked one — is under 75%, its row is the lead-sheet row through the demo and your three passes (continuous mode; one window in call-and-response), and goes back to chord blocks (and one pass) once the key climbs back over the floor. It's the same rule as any session — Deep Practice just tends to put that key at the head, where the demo is. When it isn't at the head, the sheet is on screen and lit for the whole key before it, so you read ahead.
