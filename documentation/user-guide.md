...
There's **no sheet music during a session** — you read the changes, not the line — with one exception: the key you're learning, while it's beating you. The key you're learning is the one most recently unlocked (the earlier ones you learned before the next one arrived). Once your rolling score in that key drops under 75%, its row in the stack turns into a lead sheet: the line engraved with its changes above the staff, in your instrument's written pitch, the note the band is at lit as it goes and the current bar marked under the staff. And you play it **three times in a row** — read it, read it again, then from memory — with the sheet holding still through all three; only the third pass counts toward your score and the key's progress, the first two are rehearsals (each still shows its percentage). Bring the key back to 75% or higher and the row goes back to chord blocks and a single pass on its own. A key you've never played is always by ear first — the sheet only ever answers a miss — and once all twelve keys are unlocked there's no sheet at all. There's nothing to switch on or off. If you'd rather study the line before you start, it's still on the lick's page.
...
- **The sheet music is conditional too.** While the key you're learning — the most recently unlocked one — is under 75%, its row is the lead-sheet row through the demo and your three passes, and goes back to chord blocks (and one pass) once the key climbs back over the floor. It's the same rule as any session — Deep Practice just tends to put that key at the head, where the demo is.
...
The session already does the gentle thing on its own: the key you're learning shows its sheet music, three passes at a time, while you're holding it under 75% (see [What happens during a session](#what-happens-during-a-session)). Reset is for when reading the line isn't the problem.
