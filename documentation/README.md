...
> **2026-09-10** — **the dev server stops re-initialising Sentry.** A long-running `npm run dev` printed Node's `MaxListenersExceededWarning` ("11 error listeners added to [ChildProcess]"). Sentry's server init is not idempotent — every call adds another child-process watcher, another crash handler and, on macOS, another `sw_vers` spawn whose child carries every watcher's error listener — and in dev SvelteKit loads `src/instrumentation.server.ts` through Vite's SSR loader on every request, so each wipe of the module graph ran it again in the same process: a Vite restart (`vite.config.ts`, `.env`, `svelte.config.js`), an edit to the file or its import, and any `tsconfig.json` event under the project root — which, with worktrees nested under `.claude/worktrees/`, means every parallel session's checkout. The 13-hour process that raised the warning had initialised Sentry 48 times and reached 55 during the investigation. The init is now guarded by `isInitialized()`: one client per process, and an edit to the Sentry options in dev needs a dev-server restart — [Tech Stack](./architecture/tech-stack.md).
>
> **2026-09-09** — **production moved, and the server stopped shouting at scanners.** The site now runs on a 4 GB droplet; the deploy pipeline had to be pointed at it, and the first deploy after the move landed on the retired box until the public health check caught it. Separately, every probe for a path that doesn't exist used to write a fifteen-line stack trace to the server's error log, which had reached 320 MB. The server now logs nothing for a client's mistake and one entry, with the request line, for a real fault (`createServerErrorHandler`) — [Tech Stack](./architecture/tech-stack.md).
>
> **2026-09-08** — **two ear-training takes scored right.** Two correct takes on the metronome came back marked down. *Climb to Five*: a re-tongued G3 speaks on its second harmonic for the first 70 ms, and that transient had been cut into a note of its own and scored as a G4 — a short note an octave above the note before it, with the fundamental still present around it, now keeps its attack and takes the lower octave. *Blue Note Drop*: the downbeat click you come in on sits inside the pre-armed capture, and your first note's own attack used to vouch for the click's onset, splitting the A3 in two; an onset now only counts on readings that don't already reach the next attack. The same take's second G3 was tongued on beat 3 under the click and never registered — a new acceptance shape catches a horn that audibly stops across a hole and then holds its level, which a click cannot fake. Still open: a ride click on a held note can read as a feather tongue, and the click grid the segmenter is handed has been ~0.3 s off the real clicks since the capture was pre-armed (`validateOnsets`, `RE_ARTICULATION_GAP_BAND_STOP`) — [Audio Pipeline](./architecture/audio-pipeline.md).

> **2026-09-07** — **the chord stack shows while the band loads.** A lick-practice session used to open on an empty stack until the instrument had loaded — on the Daily path that is 307 sample decodes (the sax set plus the backing kit), which on a slow connection is the whole download. The first lick's rows, lead sheet included, now build the moment setup begins, so you can read the chart and the line while the samples come in; the audio starts underneath as before. Found through CI: three builds in a row timed out waiting for a row on a contended runner, a rerun of the same commit passed, and a one-core throttle of the same test showed the mic live at nine seconds and the row at fifty-seven (`initializeSession`) — [State Management](./architecture/state-management.md).
>
> **2026-09-06** — **no empty slot above the first key.** The lick-practice key stack parked every row one chart-row below the top of its viewport, including the first — which had no previous row to fill that slot, so the first key of every lick (and every Deep Practice cycle) opened under a blank band the height of a chart row. The first row now sits flush with the top; from the second key on nothing changes — the row you just played stays above the one you're playing and the stack steps from then on (the first key boundary moves only the highlight). The small lick-name label that hung in that band is gone; the header's title names the lick. Licks with one or two unlocked keys keep the same fixed-height viewport, so their spare space now sits below the rows rather than above; and that viewport is now reserved for a lead-sheet row whether or not the stack has one, so the ring under it never moves — it used to shift 2 px when a stack gained or lost its sheet (`keyStackLayout`) — [User Guide](./user-guide.md#what-happens-during-a-session), [Components](./api-reference/components.md).
>
> **2026-09-03** — **no demo on a refill cycle.** Deep Practice no longer plays the lick to you after you've cleared the whole rotation: the tempo-bump cycle (and the focus drill's step-up and re-admission cycles) opens straight on your worst key. The per-key rolling score lags a clear — a 95% from a 70% history lands at 80% — so the old "demo while the head key is under 90%" rule replayed a line you had just played; the score rule still applies to every other cycle. A revealed key that opens a no-demo cycle gets the two-bar reading pause in the demo's place — [User Guide](./user-guide.md#what-happens-during-a-session), [State Management](./architecture/state-management.md), [state API](./api-reference/state.md).
>
> **2026-09-03** — **the sheet waits its turn, and a reading pause heralds it.** The lead-sheet row no longer shows a key early: while the previous key plays, its row is that key's chord chart (the staff is engraved underneath but hidden), and when the previous window closes the band drops into a **two-bar reading pause** — a ii-V vamp into the new key, a new `read` phase on the tab (READ, then *Play G in 4·3·2·1*) — while the row steps into place and the chart cross-fades into the staff. Only then does the first pass open. A revealed key that opens a cycle with a demo needs no pause: the demo is its herald (on a no-demo refill cycle it gets the pause instead — see the entry above). The read-ahead parking from earlier today is withdrawn; the stack is back to one rule. Display position (row, pass, beat) is now read off the scheduled window plan (`cyclePositionAt`), which is what keeps every key's beat aligned across a pause of any length (`LEAD_SHEET_PAUSE_BARS`, `getKeyPauses`, `planCycleWindows.pauses`, `lickAudioBars.pauseBars`) — [User Guide](./user-guide.md#what-happens-during-a-session), [State Management](./architecture/state-management.md), [state API](./api-reference/state.md), [Components](./api-reference/components.md).
>
