...
> **2026-09-03** — **lead sheet readable a key ahead.** The key stack now parks the row BEFORE the lead-sheet row at the top, so the whole sheet is on screen and lit for the preceding key and nothing moves at its own downbeat. Until now a row only came fully into view as it became the active row, on the same tick its play window opened — and in Daily/Focused sessions the key being learned is always the LAST row, so the sheet arrived half-clipped (107 of 212 px) and dimmed exactly as the mic opened. Viewport height unchanged; the just-played row scrolls out at that step (`keyStackLayout` read-ahead, `.row.ahead`) — [User Guide](./user-guide.md#what-happens-during-a-session), [Components](./api-reference/components.md), [State Management](./architecture/state-management.md).
>
