...
> **2026-09-01** — **sheet music when a key is beating you.** Lick-practice sessions (Daily, Focused, Deep) show the current key's notation under the chord blocks while its rolling score is under 75%, and hide it again once it recovers — never on a first attempt, never for tricks, no setting (`getNotationReveal` / `shouldRevealNotation`) — [User Guide](./user-guide.md#what-happens-during-a-session), [State Management](./architecture/state-management.md), [Components](./api-reference/components.md).
>
