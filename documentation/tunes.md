...
- **PDF + OMR transcription** (optional, for the technically inclined): if you run the local OMR tool on the chart first (`python -m omr transcribe chart.pdf` — see `docs/omr/` in the repository), you can attach the resulting `.omr.json` above the PDF picker. Lines the transcription covers get their melody from a dedicated music-reading model — measurably more accurate than the AI reader on typeset charts — and skip the AI call entirely; chord symbols still come from the page's own text. Lines it can't cover fall back to the AI as usual.

Whatever route you take, the chart is stored in its **real concert key** — unlike licks, which are all stored in C. A tune in F stays a tune in F, and the key selector transposes it for reading.
