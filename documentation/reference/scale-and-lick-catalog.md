...
The `PhraseCategory` type defines eighteen non-user categories (plus `user`), and the shipped curated catalog populates all of them. Each corresponds to a harmonic context. The sections below walk through the largest hand-curated groupings; the remaining categories — Short ii-V-I (Maj), Short ii-V-I (Min), V-I (Maj), V-I (Min), Major Chord, Dominant Chord, Minor Chord, Diminished Chord, Digital Patterns, and Enclosures — are smaller chord- and pattern-focused collections that round out the catalog.

Beyond the per-category files, three additional curated collections feed into `ALL_CURATED_LICKS`: **Major 4th & 7th** (`major-4-7.ts`, 40 licks) and its **volume 2** (`major-4-7-vol2.ts`, 40 licks), which are tagged primarily into the ii-V-I Major and Bebop Lines categories (with some Digital Patterns and Major Chord entries), and the blues **blue-note** collection (`blues-blue-note.ts`, 75 licks) in the Blues category.
...
12-bar blues vocabulary, mostly over a static C7 with some I–IV variants (C7 → F7 in the right places). The bulk of these live in two files — `blues.ts` (120 licks) and the `blues-blue-note.ts` blue-note collection (75 licks) — with the remainder from beginner cells and the combinatorial generator.
