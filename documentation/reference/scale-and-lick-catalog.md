...
A catalog of every scale Mankunku knows and every category in the lick catalog, with notes on what each is used for in the music. This is reference material — flip to it when you need a quick reminder of what Lydian Dominant works over, or whether a Bird-style line lives in the Bebop or ii-V-I category.

## Scales

The app's scale catalog holds 33 scales in seven families. Within each family, scales share a parent key (the major modes are all rotations of one major scale; the melodic minor modes are all rotations of one melodic minor scale; and so on).

Not all 33 are equally visible. The **Scales** page in the app shows twenty of them (marked ● below), grouped by family with their degree formulas. The daily key draws on twelve scale types, which unlock one after another — Major Pentatonic, Minor Pentatonic, Major (Ionian), Blues (Minor Blues), Dorian, Mixolydian, Minor (Aeolian), Lydian, Melodic Minor, Altered, Lydian Dominant, Bebop Dominant; see [The Daily Key](../architecture/tonality-system.md). The rest are there for the licks and the backing harmony.
...
### Beginner Cells (55 licks, filed under Pentatonic and Blues)

Two- and three-note minimal cells, designed for difficulty levels 1–5. The on-ramp into the catalog — short enough that beginners can hear them, recognize them, and play them back without getting overwhelmed.
...
- 5 blues 2-note intervals

### ii-V-I (Maj) (66 hand-written, 120 in all)
...
### Blues (205 hand-written, 257 in all)

12-bar blues vocabulary, mostly over a static C7 with some I–IV variants (C7 → F7 in the right places). The bulk of these come from two collections — the main blues set (120 licks) and the blue-note set (75) — with the remainder from beginner cells and the combinatorial generator.
...
### Bebop (37 hand-written, 79 in all)
...
### ii-V-I (Min) (15 hand-written, 83 in all) — minor-key

The minor counterpart to ii-V-I (Maj). In C minor: Dø7 → G7alt → C-7 (the curated lines; the Lick Practice backing plays the V as G7♭9, from Phrygian Dominant). Different scales — Locrian Natural 2, Altered, Aeolian — and a darker, more chromatic color. These licks are stored in C minor and labelled as minor keys.
...
Almost the whole catalog sits between 1 and 70: 25 licks reach the Advanced band (61–70), a single hand-written line reaches Expert (71–80), and nothing is rated higher. The Licks page itself filters only by free-text search and by progression (over your own book), not by difficulty band — the band table above is just the reference for reading a lick's difficulty label. (Community browsing does have a *Max difficulty* filter.)

A lick's band shows on its detail page, in a colour that runs from green (easy) through amber to brick red (hard). Your Tonal Mastery and scale levels borrow the same ten names — Beginner to Virtuoso — on a teal-to-brass colour ramp instead, so that high reads as achievement rather than danger.

## Combinatorial licks

In addition to the hand-written licks, the catalog includes 471 phrases generated by pairing a set of **scale patterns** (sequences of scale degrees) with **rhythm templates** (durations and offsets). Only musically valid pairings get produced: the shape has to fill the rhythm exactly — once, or repeated two or three times, each repeat allowed to step up or down the scale like a sequence — and its scale family has to suit the pattern.

The combinatorial licks fill out the difficulty bands with variations the curators wouldn't have written by hand because they'd be tedious to write out one at a time. Internally they carry `source: 'combined'` (and a `combined` tag), which distinguishes them from the hand-written phrases. They're part of the curated ear-training catalog, which the personal Licks page doesn't surface, so there's no in-app toggle to filter them out.
