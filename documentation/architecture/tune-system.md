...
- **Pickup bars.** A tune's anacrusis is a FULL bar on the timeline (melody right-aligned behind a silent lead-in, so playback, the backing grid, the playhead and every `bars × barLength` consumer stay uniform) and a SHORT bar in the engraving. `music/pickup.ts` is the one resolver: `resolvePickupLength` returns the explicit `TuneSection.pickupLength` when the melody agrees with it, else — only for the legacy import shape, an unlabeled one-bar first section with a form after it — the length inferred from the first pitched beat, so tunes imported before the field existed render right with no re-import. Given a length, the renderer starts the gap-fill at the silent prefix (stored rests inside it are dropped, one straddling it is clipped), shortens the chord voice's spacers to match, gives the partial bar no bars-per-line column (a pickup-only section shares the first system with the section it leads into), emits `[I:setbarnb 1]` after its barline because abcjs counts an anacrusis as bar 1, and hands the pickup line the boxed letter of the following section — abcjs draws part labels only at a line start — which `NotationDisplay` then nudges over the first full bar, as MuseScore prints it. A field that contradicts the melody falls back to the full bar. The OMR bridge (`tunes/import/omr-transcription.ts`) mirrors the Python validator: a short FIRST measure with a form after it is the anacrusis — right-aligned into a full bar and carried with its exact length as `ModelBar.pickupBeats` (which `claude-pdf.ts` prefers over the first-beat derivation) — while a short later measure is a misread flagged for review, except the final bar complementing a pickup; LEGATO itself tends to write a pickup as a full measure behind a leading rest, which the assembler's late-onset rule already catches (pinned on the recorded Donna Lee run).

Layout policy lives in pure modules beside it:

- **`pickup.ts`** — the anacrusis resolver above, plus the editor's option list (every eighth-note multiple inside the bar) and beat labels.
- **`chart-layout.ts`** — density-aware reflow (`suggestBarsPerLine`, 3–6 bars per system, 4 by default), jazz slash notation for melody-silent bars (never a partial pickup bar), multi-rest runs, `CHART_STAFF_WIDTH`.
