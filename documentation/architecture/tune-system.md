...
**`state/tune-practice.svelte.ts`** (runes wrapper) exposes the `tunePractice` rune, `previewSessionPlan`, `startTunePracticeSession(sheet, ppq)` → `TunePracticeAudioPlan`, the phase transitions (`setup` → `count-in` → `head` → `running` → `complete`), window bookkeeping (`expectedForWindow` / `trickForWindow`, `recordWindowResult`), and `buildFreestyleBook`.
