...
- `startSession()`, `startDailyPracticeSession()`, `startSingleLickSession(lickId, tempoBumpPercent?)`, `startTrickSession()` — The four entry points; all converge on the same playback engine. Single-lick (Deep Practice) cycles only the lick's currently-unlocked keys, runs a **session-local tempo ramp that is never persisted** (opens 2% under the stored tempo, climbs by `config.tempoBumpPercent` per cleared rotation — 1% by default, 0.5–5% from the setup knob), and **derives its progression from the chosen lick's own `prog:*` tags** rather than `config.progressionType` — fixes the case where a major lick gets stuck over a minor vamp because the setup screen was set that way. `startTrickSession` rides the same round loop with a single `kind: 'trick'` plan item whose phrase is regenerated every round; see [Trick Scoring](./trick-scoring.md).
