...
The same rolling score drives the in-session **sheet-music reveal**. When the key stack is built (`getPlannedKeysForLick`, at lick or cycle start) every `PlannedKey` is stamped `reveal`: the key's rolling score is defined and below `KEY_FLOOR_THRESHOLD` (0.75) — `shouldRevealNotation` in `state/lick-practice-rotation.ts`, which unlike `shouldDemoHeadKey` treats an unknown score as *no* reveal, so the first attempt in any key is by ear. Decided once per stack and never re-derived mid-cycle, because a revealed row is TALLER and a row's height must not change while the stack scrolls. The rule is the same in both directions: the row engraves after a sub-floor attempt and returns to chord blocks once the EWMA recovers, in every session type (Daily, Focused, Deep; continuous or call-response). `UpcomingKeysDisplay` renders a revealed row as a **lead-sheet row**: `leadSheetTuneFor` (`music/lead-sheet.ts`) wraps the row's phrase as a one-section `Tune` for `NotationDisplay tune=` (`frameless`, a wide `staffWidth`, and `tuneOptions` — the phrase's mode, one stretched system, no bar number — through `tuneToAbc`, which engraves the chords), the note at the current beat lit via `cursorIndex` (`music/beat-cursor.ts`), and a dots-only `ChordChart` beneath as the beat strip — no caption, by decision: the engraving is the message. Row heights are fixed (chord row / lead row) and the pure `keyStackLayout` (`ui/key-stack-layout.ts`) holds the active row one slot below the top (the previous row's height) for its whole key and steps at each boundary — the stack never drifts, because an engraved staff crawling upward a pixel per frame strobes, whether the translate is fractional or snapped — and reserves the viewport for two tall rows so it never resizes between cycles. Trick items never reveal, nothing is persisted, and there is no setting.
...
The pure policy — `sortKeysWorstFirst`, `shouldDemoHeadKey`, `shouldRevealNotation`, `resolveNextCycleStart`, `planCycleWindows`, and the focus ramp's `focusStartTempo`, `focusStepDownTempo`, `planFocusRamp`, `resolveRampCycle` — lives in `state/lick-practice-rotation.ts` so it can be tested without a transport.
