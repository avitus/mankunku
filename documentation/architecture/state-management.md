...
The same rolling score drives the in-session **notation reveal**. `getNotationReveal()` names the current key while its rolling score is defined and below `KEY_FLOOR_THRESHOLD` (0.75) — `shouldRevealNotation` in `state/lick-practice-rotation.ts`, which unlike `shouldDemoHeadKey` treats an unknown score as *no* reveal, so the first attempt in any key is by ear. The rule is the same in both directions: the sheet appears after a sub-floor attempt and withdraws once the EWMA recovers, in every session type (Daily, Focused, Deep; continuous or call-response). The session route mounts `NotationReveal` — a `NotationDisplay` of the route's memoized `currentPhrase`, gated on the key so abcjs re-renders only when the key changes — under the chord stack, hidden during the inter-lick score hold. Trick items never reveal, nothing is persisted, and there is no setting.
