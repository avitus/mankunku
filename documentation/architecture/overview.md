...
- A **Practice** button on any lick's detail page in your book launches **Deep Practice** on that single lick — a focused, *continuous* round through just that lick's currently-unlocked keys. No rest bars, no round card: a one-bar ii-V turnaround joins each cycle to the next, the rotation is re-sorted so your worst key comes up first, keys you clear at 95% drop out, and the app stops demonstrating the lick to you once you're holding 90% on the key at the top. Clear the whole rotation and the tempo bumps 5 BPM and it refills.
