...
- A **Practice** button on any lick's detail page in your book launches **Deep Practice** on that single lick — a focused, *continuous* round through just that lick's currently-unlocked keys. No rest bars, no round card: a one-bar ii-V turnaround joins each cycle to the next, the rotation is re-sorted so your worst key comes up first, keys you clear at 95% drop out, and the app stops demonstrating the lick to you once you're holding 90% on the key at the top. It opens 2% below the lick's saved tempo; clear the whole rotation and it bumps 1% and refills. The ramp is session-local — Daily Practice still finds the lick at the tempo it was graded at. The report's **Next** card is the other way in: when it names a key that came in under 75%, its button launches the **focus drill** — that key alone, worked back up to the saved tempo, then the rest of the rotation rebuilt one key per clear, weakest first.
