...
- It listens through your microphone while you play it back — the mic is open before you start, so you come in when you're ready and only your playing is scored, not your reaction time.
- It scores how close you got on pitch and rhythm, and either advances you or hands you one retry (a second miss moves on anyway).
...
- Tempo adjusts once per lick, from your average across the keys you played that session: +2 BPM at ≥ 95%, +1 at ≥ 90%, easing down by 1 in the 75–89% band, and −3 below 75%. (Single-lick Deep Practice runs its own rule instead: it opens 2% below the lick's saved tempo, bumps by the Tempo Bump setting (1% by default) only once you clear every currently-unlocked key in a round, and saves none of it. Launched from the report's weak-key recommendation it runs the focus drill: that key alone, 10% under, on a staircase — up per clear, down three steps per sub-75% attempt — until it's back at the saved tempo, then the other keys one per clear, weakest first, tempo held.)
...
- **Daily Practice** — the default session type — rotates across every progression you have tagged licks for, sized to your time budget: a daily warm-up that touches every line in your book. A **Focused Session** pins one progression instead.
- A **Practice** button on any lick's detail page in your book launches **Deep Practice** on that single lick — a focused, *continuous* round through just that lick's currently-unlocked keys. No rest bars, no round card: a one-bar ii-V turnaround joins each cycle to the next, the rotation is re-sorted so your worst key comes up first, keys you clear at 95% drop out, and after the opening demo the app stops demonstrating the lick to you once you're holding 90% on the key at the top — and never on a refill cycle, the one that follows clearing the whole rotation. It opens 2% below the lick's saved tempo; clear the whole rotation and it bumps (1% by default), refills, and opens straight on your worst key. The ramp is session-local — Daily Practice still finds the lick at the tempo it was graded at. The report's **Next** card is the other way in: when it names a key that came in under 75%, its button launches the **focus drill** — that key alone, worked back up to the saved tempo, then the rest of the rotation rebuilt one key per clear, weakest first.
...
- A drill runs like Deep Practice: continuous cycles over a two-bar vamp (major, dominant, or minor, whichever the variant wants), starting on your written C at 60 BPM and earning keys and tempo as you clear them. Enclosures drill a five-bar figure — a pickup bar of approach notes, then four landings on four downbeats; triad pairs a one-bar cell.
- It plays you an example whenever there's something new to hear — the first round, and any round whose style you haven't heard yet this session — but **you are not being asked to play the example back**. You're being asked to play the device. Score it and the app checks whether your notes belong to the formula, not whether they match the demo. Triad pairs go further: play the pair as the demo cell, as alternating triplets, or as four eighths per triad, and the app takes the best of the three readings.
...
- At each insertion point the app suggests licks that fit that progression in that key — ranked by what you've actually unlocked and drilled; in Suggest mode it only names licks you already have in that key, so it never asks for a key you haven't earned. Trick variants you've starred join the same list over vamps and ii-V-Is, ranked behind the licks tagged for the spot.
...
- **The daily key** rotates — every two or three days at first, daily once you've unlocked seven or more tonalities (more on this in [The Daily Key](./tonality-system.md)). Side A defaults to today's key but lets you override; Side B and Tricks ignore the daily pick and run their own per-item key ladders, earning more keys toward the full 12 across successive sessions.
...
1. The app picks (or generates) a phrase. On Side A, it's drawn from the catalog, your own licks included, and transposed into today's key. On Side B, it's the current tagged lick transposed into the current key. On Tricks, it's a fresh realization of the device's formula.
2. The phrase plays through speakers or headphones — over the click alone on Side A, over a count-in and the rhythm section everywhere else.
3. After the phrase ends, the beat keeps going and the app is listening through your microphone. On Side A the mic opens before you play, so your first note's attack is always caught and the lead-in before it is discarded; in the continuous modes your window opens on a bar line.
