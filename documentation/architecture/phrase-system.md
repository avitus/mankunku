...
- **Your own licks.** Anything you step-enter (`/entry`) or record (`/record`) joins the library tagged as user content. They behave the same as curated licks — searchable, tag-able for Side B, transposable to any key.
...
- Joins the library with a `user` source.
