...
### `trimToPerformance(readings, workletOnsets, duration, preroll?, weakReadings?): TrimmedCapture`

For **reacted** entrances (ear training — the user comes in on their own reaction time). Drops the runs of readings that never reach performance level (see `dropSubFloorRuns` below), then everything more than `preroll` ahead of the first surviving reading, and rebases what is left to the new origin. `PERFORMANCE_PREROLL_SECONDS` (0.35) is bounded on both sides: it must exceed the ~190 ms detection lag it undoes, and stay under the ~250 ms where DTW alignment starts to flip (`rhythmDistance` saturates at one beat; 12 of the 21 scored diagnostic fixtures change grade between a 0.25 s and 0.5 s lead-in). Deliberately a **pure function of the capture** — gate, then first surviving reading minus a constant — so the live path, the authoritative replay rescore and /diagnostics (for ear-training recordings — see `diagnosticsReplayFrame`) all derive the same offset without persisting one. Returns `{ readings, weakReadings, workletOnsets, duration, offset }`; `offset` is the seconds removed from the front (add it to `recordingTransportSeconds` to keep bleed evidence on the beat grid). `weakReadings` (the detector's sub-threshold frames, default none) ride the same offset with the lead-in dropped and are not floor-gated — they are only read inside the holes of `readings` (`findGhostNotes`); pass them whenever the result is segmented. Captures with no readings, or recorded before pre-arming existed (first reading already at ~0), come back untouched.
...
interface PitchReading {
  ...
  weak?: true;           // Sub-threshold frame (clarity in [WEAK_CLARITY_FLOOR, threshold)) — side channel only
}
```
...
A **weak** reading is a frame whose clarity sits in [`WEAK_CLARITY_FLOOR` (0.5), threshold). Detectors never mix them into the confident stream: they come back on a side channel (`getWeakReadings()`, `ReplayResult.weakReadings`) whose only consumer is the ghost-note pass (`findGhostNotes`). A weak reading bypasses the octave stabilizer — `midi`/`midiFloat` are the raw pick after subharmonic correction — and carries only `rms`, `hfRms` and `rmsMin` of the optional fields.
...
interface PitchDetectorHandle {
  ...
  getWeakReadings: () => PitchReading[];  // Sub-threshold frames, in time order — never mixed into getReadings()
  clear: () => void;                      // Empties both streams (start() does too)
...
- Clarity threshold: `CLARITY_THRESHOLD = 0.80`; frames from `WEAK_CLARITY_FLOOR` (0.5) up to it are kept as weak readings
...
One analyser window → `{ reading: PitchReading | null, rawClarity, weakReading? }` (`rawClarity` is always present, for UI meters; `reading` is null below the clarity threshold or out of range; `weakReading` is set, without touching the stabilizer, when an in-range frame's clarity is at least `WEAK_CLARITY_FLOOR` (0.5) but under the threshold — the 2026-09-16 ghosted Cs read 0.54–0.76, the same take's click bursts and breath noise 0.35–0.49). Runs Pitchy, lifts an octave-down subharmonic pick with `correctSubharmonic` before the frequency enters the MIDI stream, measures the envelope/timbre fields (`rms`, `rmsMin`, `bandRmsMin`, `hfRms`, `shapeBreak`), applies the octave stabilizer (pass `null` to skip it), and flags `octaveUp` only on a frame neither the subharmonic correction nor the stabilizer moved. `FrameOptions` is `{ sampleRate, clarityThreshold?, minFrequency?, maxFrequency?, windowAnchor? }`; the thresholds default to `DEFAULT_CLARITY_THRESHOLD` (0.80), `DEFAULT_MIN_FREQUENCY` (80) and `DEFAULT_MAX_FREQUENCY` (1200).
...
### `segmentNotes(readings, onsets, recordingDuration, minNoteDuration?, onsetGuard?, minReadings?, workletOnsets?, bleedOnsets?, articulationOnsets?, weakReadings?): DetectedNote[]`
...
| `onsetGuard` | `number` | `0.08` | Seconds after a segment start during which FFT-tainted readings from the previous note (frames still reading its MIDI) are skipped |
...
| `weakReadings` | `PitchReading[]?` | — | The capture's sub-threshold frames, same time base as `readings`. When supplied, ghosted notes recovered from them (`findGhostNotes`) are carved into the result as the last step. Ear training (live and rescore), lick practice, tune-practice scored windows and /diagnostics pass them; record-a-lick and the tune-practice freestyle scan do not. |
...
2. For each segment, skip readings inside `onsetGuard` — but only when the boundary IS an amplitude onset (worklet or articulation); a pitch-derived boundary already starts where the readings show the new pitch, and guarding it ate the frames that define the note (2026-07-25 root-frame). Even then only frames reading the PREVIOUS note's MIDI are skipped — a frame on another pitch is the new note, and may be all of it (the 2026-09-16 A3: five frames, all inside the window, before the downbeat kick blanked tracking). Split on durable pitch changes (legato) and fold a sub-segment exactly an octave from its neighbour into it when it is shorter than 150 ms or ≥ 25% of its raw picks read the neighbour's pitch (both only into a neighbour longer in span), or — the **octave-cracked attack** — when the neighbour follows it, the stabilizer never confirmed its octave (fewer than `OCTAVE_CONFIRM_FRAMES`, 3, frames past warmup whose raw pick is its pitch; warmup frames are raw passthrough and the frames the confirm inertia holds after the raw pick has moved are not audio) and the raw pick sat in the neighbour's octave on more frames than in its own. Reference: the 2026-09-16 sharp-9-flat-9-dom first D, a tenor D4 whose attack spoke as D3 for ~110 ms before the octave vent took over, with no attack at the switch — 190 ms from the downbeat click's onset to the reported switch (past the 150 ms rule), two upper frames in nine (under 25%); replay emitted D3 @0.093 then D4 @0.283. The 25% rule folds the same crack when the head confirms at most one frame, which is how the 2026-08-11 curl-to-the-floor and blue-note-climb attacks were already folded. A slurred leap from a real lower note confirms it (a straight eighth at 129 BPM, ~9 frames past warmup). The crack rule counts frames, not span: a span credits the crack with the click lead-in and the inertia frames, and the live detector's window-end stamps lengthen a head sub by a whole analyser window. Warmup frames exist only after a stabilizer reset — at every worklet onset in replay, lick practice, record-a-lick and tune-practice windows, but only at the capture start in live ear training (whose authoritative rescore is a replay). Then pick the pitch by a clarity-weighted pitch-class vote with a nearest-octave tie-break (warmup frames down-weighted), median cents, average clarity. Drop segments shorter than `minNoteDuration` or made only of warmup frames — unless they hold a completed warmup window (`WARMUP_FRAMES`, 5) all on one MIDI, where the stabilizer's vote finished; one with 2 to `minReadings − 1` readings takes its single clearest reading (clarity halved) rather than vanishing.
...
9. **Ghost notes** (only with `weakReadings`) — each `findGhostNotes` result is carved out of the notes it overlaps: a note straddling it keeps its head and tail, a note starting inside it starts where the ghost ends, fragments under `minNoteDuration` are dropped. Last, so no merge pass reaches across a ghost.
...
### `findGhostNotes(readings, weakReadings): DetectedNote[]`

Recovers **ghosted** notes — half-fingered, breathed rather than blown — that read under the clarity threshold for their whole length and so leave only a HOLE in the confident stream. Reference: the 2026-09-16 sharp-9-flat-9-dom take (Deep Practice), D C D C D C B A G# with the three off-beat Cs ghosted 9–12 dB under the Ds at clarity 0.54–0.76; every C vanished into the Ds and a correct take saved as 6 of 9. A hole between consecutive readings of `GHOST_MIN_HOLE` (0.075 s) to `GHOST_MAX_HOLE` (0.4 s) yields a ghost when its weak frames hold a **plateau**: at least `GHOST_MIN_FRAMES` (3) consecutive frames (≤ 0.04 s apart), folded into the line's register (a breathy ghost flips to its doubled period), all within `GHOST_PLATEAU_TOLERANCE` (±0.35 st) of their median. The plateau must sit `GHOST_MIN_FLANK_DISTANCE` (0.75 st, ignoring octave) or more from the confident note on each side and at `GHOST_MIN_LEVEL` (−20 dB) or more of the louder of them. Up to `GHOST_MAX_ABSORBED` (2) confident frames within `GHOST_ABSORB_TOLERANCE` (0.5 st) of the plateau are the ghost's own (the reference's third C cleared the threshold on one frame); a longer run is the next note itself, and no ghost is emitted. The ghost runs from its first frame (or first absorbed frame) to the next note's first reading. Its pitch is the plateau median, **kept as measured** — `midi` the nearest semitone, `cents` the rest — and the note carries `ghost: true` (shown on /diagnostics). The scorer judges it like any other note: the reference take's first two Cs measure C + 70 and + 62 cents, round to C#, and score as wrong notes; the third (+ 43) is a C. Replayed across the 34 fixture takes, only the three ghosted Cs qualify; every other hole's plateau sits within 0.72 st of a neighbour (pinned per fixture in `pitch-replay.test.ts`).

**The bleed evidence is load-bearing, and the caller supplies it.** `resolveOnsets` takes no bleed argument, so a click can and does survive as a base onset; what removes the resulting split is `bleedOnsets` reaching `findReArticulations` and `mergeSamePitchWithoutAttack` (via `segmentNotes`). Dropping it at a call site silently restores the phantom — `tests/integration/pitch-replay.test.ts` scores the same take both ways. The same holds for `weakReadings`: a call site that drops them silently loses every ghosted note (pinned on the 2026-09-16 take).
