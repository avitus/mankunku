...
Stepped preview strip showing the previous, current, and upcoming key chord charts. The current row holds one slot below the top for its whole key (the just-played row and its score flash stay visible above it) and the stack steps by that row's height at each key change, eased by a CSS transition — it never drifts, because an engraved staff crawling a pixel per frame strobes. The one exception is **read-ahead**: when the NEXT row is the lead-sheet row, the current row parks at the top so the whole sheet sits inside the viewport for the preceding key, lit (`.row.ahead` — opacity only; the playhead, beat and ring stay `.current`-gated), and the stack does not move at the sheet's own downbeat. The just-played row scrolls out at that step, its score flash only glimpsed (the ring keeps the score). Row key labels read in each planned phrase's mode (`keyLabel(written, lickMode(pk.phrase))` — "Dm" on a minor drill) and the embedded `ChordChart` gets the same `mode`.

A row whose `PlannedKey.reveal` is set (the key being learned — the most recently unlocked — is under the floor) renders as a **lead-sheet row** (`data-testid="lead-sheet-row"`): `leadSheetTuneFor` (`music/lead-sheet.ts`) wraps the row's phrase as a one-section Tune for `NotationDisplay` (`frameless`, `staffWidth` 640, `tuneOptions` from `leadSheetAbcOptions` — the phrase's mode, one stretched system, no bar number), the current bar marked ON the staff via a `playhead` `rangeMarker` (drawn by NotationDisplay from abcjs's bar geometry, so it aligns with the engraved bars by construction; the marker array is derived off an INTEGER bar so it only changes identity when the bar does) — no caption, by decision. The SVG is sized by **width** (abcjs's responsive inline styles overridden with `!important`: static position, no aspect padding box), so `staffWidth` is the zoom — ~1.45× at a 960 px row — capped at the staff box's height so a phrase with ledger lines scales down rather than clipping. Lead sheets are built once per stack so abcjs engraves each row once. Rows are fixed heights (105 px chord row, 212 px lead row = a 200 px clipped staff box + row padding) and `keyStackLayout` (`ui/key-stack-layout.ts`) holds the active row one slot below the top for mixed heights — or at the top when the next row is taller than the slot and the three would not fit (read-ahead) — and reserves the viewport for the tallest row plus its neighbour so the ring below never moves. A revealed row plays `passes` windows in ONE held row — 3 in continuous mode, 1 in call-and-response, where each window already replays the app's half: the route maps the transport's slot-unit scroll onto row units with `rowScrollFraction` before the layout, and the PLAY tab reads `Play · n/3` (`data-pass="n"`), the pass derived from the fraction within the row.

| Prop | Type | Description |
|---|---|---|
| `plannedKeys` | `PlannedKey[]` | All keys for the current lick |
| `scrollFraction` | `number` | Scroll position in **row** units: the integer part is the row being played, the fraction the position within it — for a multi-pass row, `floor(fraction × passes) + 1` is the pass. The session converts the transport's slot-unit progress with `rowScrollFraction` (`ui/key-stack-layout.ts`) before passing it, so a three-pass row holds through all its passes; a caller passing raw key units would move that row after its first pass. |
