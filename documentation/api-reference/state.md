...
- `getKeyPauses(lickIdx): number[]` — Bars of reading pause before each rotation slot's first window (same indexing): `LEAD_SHEET_PAUSE_BARS` for a revealed key in continuous mode unless it opens a cycle that demos (slot 0 follows the demo, which is its herald; on a no-demo refill cycle slot 0 pauses too), else 0. `buildLickSuperPhrase` fills those bars with `turnaroundHarmony` (a ii-V into the key, repeated — the band vamps) and the scheduler passes them to `planCycleWindows` as `pauses`.
