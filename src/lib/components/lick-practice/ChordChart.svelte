<script lang="ts">
	...
	let { harmony, currentBeat, timeSignature, isPlaying }: Props = $props();
	...
	/** Group cells into rows of bars (each row = barsPerRow worth of beats) */
	interface RowInfo {
		cells: CellInfo[];
	}
	...
	const cells = $derived.by(() => {
		...
		for (const seg of harmony) {
			...
			const symbol = chordSymbol(seg.chord.root, seg.chord.quality);
