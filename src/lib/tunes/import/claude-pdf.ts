...
import {
	...
	type BarStructure,
	type HarmonyChange
} from '$lib/tunes/section-builder';
import {
	validateAdoptedTune,
...
export function claudeJsonToTune(data: unknown): ClaudePdfConversion {
	const errors: string[] = [];
	...
	if (!barwise) {
		...
		} else if (rawSections.length > MAX_SECTIONS_PER_ADOPTED_TUNE) {
	...
	const validation = validateAdoptedTune({ ...sheet, id: 'pdf-import-draft' });
	if (!validation.valid) {
