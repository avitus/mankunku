...
export interface AdoptedLeadSheetValidation {
	valid: boolean;
	errors: string[];
}

/** DoS cap: total melody notes across all sections (Tone.js scheduling). */
export const MAX_NOTES_PER_ADOPTED_SHEET = 5000;
/** DoS cap: harmony segments across all sections. */
export const MAX_HARMONY_PER_ADOPTED_SHEET = 2000;
export const MAX_SECTIONS_PER_ADOPTED_SHEET = 64;
...
export function validateAdoptedLeadSheet(input: unknown): AdoptedLeadSheetValidation {
	const errors: string[] = [];
	...
	if (sections.length > MAX_SECTIONS_PER_ADOPTED_SHEET) {
		errors.push(`too many sections (${sections.length} > ${MAX_SECTIONS_PER_ADOPTED_SHEET})`);
		return { valid: false, errors };
	}
	...
	if (totalNotes > MAX_NOTES_PER_ADOPTED_SHEET) {
		errors.push(`too many notes (${totalNotes} > ${MAX_NOTES_PER_ADOPTED_SHEET})`);
	}
	if (totalHarmony > MAX_HARMONY_PER_ADOPTED_SHEET) {
		errors.push(`too many harmony segments (${totalHarmony} > ${MAX_HARMONY_PER_ADOPTED_SHEET})`);
	}
