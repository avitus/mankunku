...
Chronological — newest entries at the bottom (append new sessions at the end).
...
## 2026-09-11 — Test audit and docs pass: twelve partitions, a usage-limit cut, and the bugs the audit turned up

- Andy (2026-09-10 evening): remove meaningless and redundant tests, fill coverage gaps with particular attention to e2e, and thoroughly update all documentation (mine, the public docs, the user-facing docs), with as many subagents as needed.
- Plan: one agent per test partition (audio+scoring, music+notation+step-entry, state+lick-practice+tricks, persistence+cloud-sync, tunes+server+util+deploy, OMR), two e2e agents (audit existing specs / write specs for uncovered routes), four docs agents (player docs + tours + DOC_TREE blurbs, architecture, API reference, READMEs + contributing + OMR docs). Shared briefs: conservative removal criteria (meaningless or strictly redundant; never a pin named after a date/incident/take, never `it.fails`, never a golden), gap criteria (every new test must be able to fail — name the source change that breaks it), partition fences, no git state changes. A PLAYWRIGHT=1 preview served the e2e agents.
- Also re-sorted CLAUDIUS/SESSIONS.md and observations.md into one chronological order (they had become a newest-first block followed by an ascending tail); README and file headers now say "newest at the bottom".
- **The usage limit cut in** with ten agents mid-work. What survived: 168 files of partial edits on disk, 9 failing persistence tests and 10 failing OMR tests from half-finished work. Resume: checkpoint commit → merge `origin/dev` (0a904f8, the tune-practice setup restyle and `PLAYWRIGHT_PORT`) → `reset --mixed origin/dev`, so the work sat uncommitted on the new base (one conflict, observations.md, resolved by re-running the scripted sort). Rebuilt the preview on :4273 (not 4173 — another checkout could hold it). Relaunched every unfinished agent with a continuation brief: review the predecessor's `git diff HEAD` against the criteria FIRST, restore any removal that breaks them, then finish. Reviewers restored 8 wrongly removed pins (7 in state/difficulty, 1 in persistence) — removal needs a second pair of eyes.
- **Bugs found and fixed test-first** (each test failed before its fix):
  - Lead-sheet row spelling: the row engraves licks through `tuneToAbc`, whose inline chain lacked the scale tier (and read minor chord roots in major); a sweep of every curated lick × 12 keys × 5 instruments found 7,415/55,380 renders disagreeing with the lick chart. `renderElement` now calls `spellingContextAt` + `resolveUseFlats`; 0 disagreements; goldens byte-identical. Visible: Mankunku Blues bar 3 over F7 (declared blues.major) prints Ab, not G#; an unspelled #9 over an imported 7b9/7#9/7alt prints as b3. This reverses the 2026-08-22 "revisit if a report lands" note — flagged to Andy.
  - Concert keys in the UI: /progress "Next:" unlock hint and the Settings key tooltips (`formatNextKeyUnlock`, `formatKeyUnlockRequirements`). `progress-unlocks.spec.ts` was ASSERTING the bug (tenor expected concert "D — G").
  - Tour sync pushed the local set over another device's completions after a failed read (tri-state `readTourStateRow`). A local tombstone at an equal mtime dropped the lick without telling the cloud.
  - `/api/chat`'s 32 KB cap bypassed by a chunked upload (bounded reader).
  - The re-home reload fired mid-hydration (found only when the e2e console guard became automatic — it was opt-in, 22/48 specs never enabled it): aborted route imports, error handler + Sentry envelope during teardown, and the layout returned data that rendered the previous user's in-memory state. Now an auth-verdict `<meta>` + `reconcileBeforeHydration` in `hooks.client.ts` `init`; `rehome-before-hydration.spec.ts` fails on the old build on every engine.
  - Unhandled lazy imports (7 sites): the lick editor could skip `?edit=` hydration and Update the wrong lick; ear training could start half-loaded; notation stayed blank.
  - OMR parser: `<|text|>` placeholder read as a broken rhythm; `>>` collapsed to one dot; compound-meter tuplet defaults ignored; `(p::r` read as a barline + ending; `(0` crashed the parse; chord length from the longest note; spaced markers lost the bar; validation.py's pickup-complement rule (mirror of the TS fix).
