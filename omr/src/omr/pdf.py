        ...
        for i in range(len(doc)):
            ...
            if longest_pt <= 0:
                raise CorruptedInputError(
                    f"page {i + 1} of {Path(path).name} reports a zero page size"
                )
            ...
            try:
                try:
                    ...
                    # to_pil() can share the PDFium bitmap buffer — copy so the
                    # returned images outlive the document and its bitmaps.
