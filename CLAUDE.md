...
- **audio/** — Everything touching Web Audio API: playback (Tone.js), capture (mic), pitch detection (Pitchy at 60fps), onset detection (custom AudioWorklet), note segmentation + quantizer, bleed filter, metronome, backing-track rhythm section (bass/comp/drums), voicings, recorder + replay.
...
- **history.svelte.ts** — Long-term daily progress summaries for calendar heatmaps and period comparisons (persisted, survives the 100-session prune window)
- **library.svelte.ts** — Lick browser filter state (not persisted)
- **lick-practice.svelte.ts** — Multi-key lick-practice flow: progression plans, per-key results, tempo adjustments, gradual key unlocking (each unlock requires avg session score ≥ 0.90 AND newest-key `passCount` ≥ 2; keys appear in alternating sharp/flat-side neighbours of the entry key on the circle of fifths). Three start paths share the engine: standard sessions pin to one progression, **Daily Practice** rotates across every progression the user has `prog:*` tags for (`startDailyPracticeSession` / `buildDailyPracticePlan`), and **single-lick mode** drills one lick until all 12 keys clear at ≥ 0.95 then bumps tempo. Licks need an explicit `prog:<progressionType>` tag to be eligible (set automatically when a lick's category matches a progression, or manually for cross-progression use). Progress persisted via `persistence/lick-practice-store.ts`.
