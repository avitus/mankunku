...
# harness). Only release ids count toward KEEP: a hand-made snapshot or a
# stray file newer than the releases (an operator's `pre-migration-*` dir)
# used to take one of the retained slots, so KEEP_RELEASES=2 kept a single
# real release. `tail -n +N` skips the first KEEP releases; the rest are
# pruned. `grep` may match nothing only in a releases/ dir with no release
# at all, which a deploy that just swapped `current` cannot be.
cd "${ROOT}/releases"
# shellcheck disable=SC2012
ls -1t | { grep -E '^[0-9]{8}-[0-9]{6}-[0-9a-f]{7}$' || true; } | tail -n +$((KEEP + 1)) | while read -r dir; do
    [[ -n "$dir" ]] || continue
    # Release IDs are generated by CI as YYYYMMDD-HHMMSS-<7-hex>. Anything else
    # is human-created (or worse) and must not be passed to `rm -rf` — the
    # filter above already keeps it out of this loop; this is the second lock.
