...
describe('CHORD_DEFINITIONS', () => {
	it('has exactly 18 chord qualities', () => {
		expect(ALL_QUALITIES).toHaveLength(18);
	});

	it('every entry has intervals starting with 0 (root)', () => {
		for (const quality of ALL_QUALITIES) {
			expect(CHORD_DEFINITIONS[quality].intervals[0]).toBe(0);
		}
	});

	it('no duplicate intervals within any single chord', () => {
		for (const quality of ALL_QUALITIES) {
	...
	it('4-note chords have exactly 4 intervals', () => {
	...
	it('5-note chords have exactly 5 intervals', () => {
	...
	it('triads have exactly 3 intervals', () => {
...
describe('chordSymbol', () => {
	...
	it('all 18 qualities produce non-empty strings', () => {
		for (const quality of ALL_QUALITIES) {
