...
describe('flattenLeadSheet — sequential concatenation', () => {
...
describe('flattenLeadSheet — repeat expansion', () => {
	it('plays a repeated section twice when expandRepeats is set', () => {
		const flat = flattenLeadSheet(
			sheet([
	...
	it('routes first and second endings correctly', () => {
		...
		const flat = flattenLeadSheet(sheet([a, e1, e2]), { expandRepeats: true });
	...
	it('keeps endings sequential when not expanding', () => {
		...
		const flat = flattenLeadSheet(sheet([a, e1, e2]));
