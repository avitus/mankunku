...
import {
	validateAdoptedTune,
	MAX_NOTES_PER_ADOPTED_TUNE,
	MAX_SECTIONS_PER_ADOPTED_TUNE
} from '$lib/tunes/adopted-tune-validator';
...
describe('validateAdoptedTune', () => {
	it('accepts a structurally valid sheet', () => {
		const result = validateAdoptedTune(validSheet());
	...
	it('rejects non-objects and missing basics', () => {
		expect(validateAdoptedTune(null).valid).toBe(false);
		expect(validateAdoptedTune('x').valid).toBe(false);
		expect(validateAdoptedTune(validSheet({ id: '' })).valid).toBe(false);
		expect(validateAdoptedTune(validSheet({ title: '' })).valid).toBe(false);
	});

	it('rejects an invalid key or time signature', () => {
		expect(validateAdoptedTune(validSheet({ key: 'H' as never })).valid).toBe(false);
		expect(validateAdoptedTune(validSheet({ timeSignature: [4, 0] })).valid).toBe(false);
	});

	it('rejects a sheet with no sections or no content at all', () => {
		expect(validateAdoptedTune(validSheet({ sections: [] })).valid).toBe(false);
		const empty = validSheet();
		empty.sections[0].notes = [];
		empty.sections[0].harmony = [];
		expect(validateAdoptedTune(empty).valid).toBe(false);
	});

	it('rejects malformed notes', () => {
		const badPitch = validSheet();
		badPitch.sections[0].notes = [{ pitch: 300, duration: [1, 4], offset: [0, 1] }];
		expect(validateAdoptedTune(badPitch).valid).toBe(false);

		const badDuration = validSheet();
		badDuration.sections[0].notes = [{ pitch: 60, duration: [0, 4], offset: [0, 1] }];
		expect(validateAdoptedTune(badDuration).valid).toBe(false);
	...
	it('rejects harmony with an unknown chord root or quality', () => {
		...
		expect(validateAdoptedTune(badRoot).valid).toBe(false);

		const badQuality = validSheet();
		badQuality.sections[0].harmony[0].chord.quality = 'power-chord' as never;
		expect(validateAdoptedTune(badQuality).valid).toBe(false);
	});

	it('rejects script-like content in text fields', () => {
		expect(validateAdoptedTune(validSheet({ title: '<script>alert(1)</script>' })).valid).toBe(false);
		expect(validateAdoptedTune(validSheet({ composer: 'javascript:alert(1)' })).valid).toBe(false);
		expect(validateAdoptedTune(validSheet({ tags: ['onload=evil()'] })).valid).toBe(false);
	...
	it('enforces DoS caps on note and section counts', () => {
		...
		tooManyNotes.sections[0].notes = Array.from({ length: MAX_NOTES_PER_ADOPTED_TUNE + 1 }, (_, i) => ({
			pitch: 60,
