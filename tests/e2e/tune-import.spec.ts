...
test('iReal link imports straight into the book', async ({ page }) => {
	...
	await page.waitForURL('**/tunes/imported-blues');
...
test('the source selector re-interprets a chart as a written-pitch part', async ({ page }) => {
	...
	// A saved tune lands on its title slug, never the minted `sheet-…` id.
...
test('a real MuseScore file imports melody and changes at concert pitch', async ({ page }) => {
	...
	await page.waitForURL('**/tunes/fly-me-to-the-moon');
...
test('a corrupt Band-in-a-Box file surfaces its warning and re-arms the input', async ({ page }) => {
	await page.goto('/tunes/import/biab');
	...
	await fileInput.setInputFiles({
		...
		buffer: Buffer.from('not a band-in-a-box song at all')
	});

	// The failure surfaces instead of dying silently…
	await expect(page.getByText(/Unrecognized Band-in-a-Box file version/)).toBeVisible();
	await expect(page.getByText('Nothing importable found in that file.')).toBeVisible();
	// …and the input value is cleared so picking the SAME file again fires
