...
test('clicking an empty bar arms the entry cursor there and typing inserts in that bar', async ({ page }) => {
	...
	await expect(page.getByTestId('entry-rail').getByText(/Bar 3, Beat 1/)).toBeVisible();
	...
	await expect(page.getByTestId('entry-rail').getByText(/Bar 3, Beat 2/)).toBeVisible();
});
...
test('entry auto-advances across the page boundary without manual paging', async ({ page }) => {
	...
	await expect(page.getByTestId('entry-rail').getByText(/Bar 6, Beat 1/)).toBeVisible();
});
...
test('clicking a rest jumps the entry cursor to that bar', async ({ page }) => {
	...
	await expect(page.getByTestId('entry-rail').getByText(/Bar 4, Beat 1/)).toBeVisible();
