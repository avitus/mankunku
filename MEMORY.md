...
- Open follow-ups: phones letterbox the width-fit staff (57 px SVG at 375 px); the stack's `{#each}` key includes the row index, so a worst-first re-sort remounts and re-engraves the sheet at each cycle boundary; the viewport moves 315↔317 between reveal and no-reveal cycles.
