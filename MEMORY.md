...
Lick-practice sessions turn a struggling key's row of the scrolling chord stack into a **lead-sheet row** (chords engraved above a full-width staff, current note lit, beat strip beneath — no caption: Andy removed it as clutter) while that key's persisted `rollingScore` is DEFINED and under `KEY_FLOOR_THRESHOLD`; same rule both ways. `PlannedKey.reveal` (stamped once per stack in `getPlannedKeysForLick`) → `shouldRevealNotation` (rotation module). Decided with Andy: unknown score never reveals (first attempt by ear — the one inversion of `shouldDemoHeadKey`), automatic only, scoring untouched, tricks excluded, nothing persisted, no explanatory caption. A separate panel below the stack was built first and REPLACED after the demo: it mounted/unmounted per key and shoved the ring 200 px, which Andy read as "a lot of flickering".

- **Never verify a practice flow in Andy's live Chrome profile.** A silent deep-practice "attempt" is a real attempt: it writes rolling score, recency and a session-log row into the same store he practices against. Use the e2e seed (`seedOnboardedAnonymous` + `installAudioMock`) with a throwaway spec (screenshots / `--headed` / video) instead; measure geometry with `getBoundingClientRect`, not a pixel budget.
- Stack geometry is the pure `keyStackLayout` (ui/key-stack-layout.ts): the active row starts where the previous row ends and slides by THAT height; the viewport reserves the TWO tallest rows so it never resizes between cycles (rows re-sort worst-first each cycle; a resizing viewport moves the ring); the stack HOLDS the active row one slot below the top for its whole key and STEPS at boundaries (420 ms eased) — it no longer drifts. The second "still flickering" report was the drift itself: five one-pixel staff lines crawling upward a pixel per frame strobe, fractional or snapped (snapping to whole pixels changed nothing — verified by re-measuring). Diagnosis tools that worked: in-page MutationObserver + per-frame transform sampling (zero DOM mutations, 0.3–0.6 px/frame). Tools that misled: whole-frame mean-luma diff (hides a small region), region pixel-change count (cannot tell motion from flicker). Rows are fixed heights (105 / 196) and a row's `reveal` is never re-derived mid-cycle.
- Engraving = `leadSheetTuneFor` (phrase → untitled one-section Tune → `tuneToAbc`, which already places chords on the spacer voice; ≤ 4 bars, windowed for long cycles) with `TuneAbcOptions` `mode`/`stretchLast`/`measureNumbers` (additive; goldens byte-identical) and NotationDisplay `frameless`/`staffWidth`/`tuneOptions`. abcjs facts: responsive mode sets `position:absolute; width:100%` inline (override with `!important`); a `T:` line reserves masthead height even when CSS hides it (hence `title: ''`); fit-by-height + wide staffwidth makes one system span the row and degrade by width on phones.
- Open follow-ups: phones letterbox the height-fit staff small (~40 px at 390 px); a playhead cursor is done (note lit at the beat) but the staff has no bar-fraction playhead.
