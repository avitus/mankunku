...
- **Session-local like everything in deep practice** — the live `FocusRamp` state, the staircased tempo and the rotation are never persisted and the lick's stored tempo/progress are untouched (`deep-practice-tempo.test.ts` + `focus-ramp.test.ts` are the contract). The one thing that outlives the session is the report's `FocusRampSummary`, which the session log keeps like any other report field (hence the `splitReportByProgression` copy below).
