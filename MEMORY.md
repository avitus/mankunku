...
- Stack geometry is the pure `keyStackLayout` (ui/key-stack-layout.ts) with ONE rule: row 0 parks at the TOP (2026-09-06 — its old slot, with no previous row to fill it, was a chart-row-tall blank band above the first key of every lick and cycle), and every later row starts where the previous row ends and slides by THAT height — it HOLDS one slot below the top for its whole key (pause + all passes) and STEPS at boundaries after the first (420 ms eased; the first boundary moves only the highlight); the viewport reserves the lead-sheet row plus a chord row whether or not this stack has a sheet (`reserveRowHeight`, passed by the component, 2026-09-07 — CodeRabbit on #246 caught the docs claiming "never moves" over the 315 ↔ 317 flip; a row taller than the reserve still wins) so it never resizes between cycles or licks (rows re-sort worst-first each cycle; a resizing viewport moves the ring). It no longer drifts: the earlier "still flickering" report was the drift itself (five one-pixel staff lines crawling upward a pixel per frame strobe, fractional or snapped). Diagnosis tools that worked: in-page MutationObserver + per-frame transform sampling. Tools that misled: whole-frame mean-luma diff, region pixel-change count. Rows are fixed heights (105 / 212) and a row's `reveal`/`passes` are never re-derived mid-cycle.
- **The sheet waits its turn, behind a reading pause (2026-09-03, third pass).** Andy: the sheet must NOT appear until the previous key has been played, and the switch from memory to reading needs a short pause to herald it. So: `LEAD_SHEET_PAUSE_BARS` (2) bars before a revealed key's first pass when it does not open the cycle (`getKeyPauses`, indexed like `getKeyPasses`; slot 0 follows the demo when the cycle has one; on a no-demo refill cycle — 2026-09-03 fourth pass, Andy: "skip the demo on any refill cycle" — slot 0 gets the pause in the demo's place; `cycleDemos()` is the one source `demoBarsForItem` and `pauseBarsFor` read); the pause is laid INTO the super phrase as `turnaroundHarmony` repeated (the cycle-join ii-V, "vamp till ready" — moved to data/progressions.ts so state needn't import audio); `planCycleWindows.pauses` → per-window `pauseTicks` → a `read` phase in `buildPhaseTimeline` (tab READ in listen-red, staff glyph, then "Play G in 4·3·2·1", never "Straight in"); `lickAudioBars.pauseBars` charges it. Two bars, like the inter-lick rest and record-a-lick's count-in; the one-bar turnaround is for a key change in the same mode. The component shows the key's chord chart as a placeholder over the engraved staff (`visibility: hidden` — engraved ahead, not shown) until the row is current, which it is from the START of its pause; the stack steps and the chart fades into the staff inside the pause. **The display reads its position off the scheduled plan**: `cyclePositionAt(tick, layout)` (rotation module) → segment / keyFraction / beat, the beat restarting at every slot so a pause of any length leaves later keys aligned, −1 through the pause; the old uniform-slot modulo (`ticks / ticksPerKey`, `beat % loopBeats`) was correct only because every segment was a multiple of the key length. `rowScrollFraction` is gone. **Read-ahead parking (`.row.ahead`, the morning's fix) was withdrawn the same day** — it answered "not completely displayed in time" by showing the sheet a whole key early, i.e. the same missing transition with the sign flipped; don't reintroduce it. The e2e third spec now pins hidden-during-C → READ + step + fade → play-in → pass 1 with nothing moving.
- Windows are per PASS, keys per SLOT: `planCycleWindows` takes `passes` (and `pauses`) and returns per-window `opens`/`closes` + `keyIndex`/`finalPass`/`pauseTicks`; `getKeyPasses(lickIdx)`/`getKeyPauses(lickIdx)` (indexed like `item.keys`, not the rows) are the single sources the super phrase and the scheduler share, like `getDemoBars`; `lickAudioBars.extraWindows`/`pauseBars` keep the Daily budget honest; `item.keys` stays deduplicated, so the ring, `sessionKeys`, `masteredThisRound` and `startInterLickTransition`'s `newestKey = keys[last]` are untouched.
- abcjs is fetched through ONE memoised loader (`notation/abcjs-loader.ts`, 2026-09-03): `NotationDisplay` reads `loaded()`/`load()`, the session route calls `load()` at mount alongside audio setup. It used to be imported per instance from `onMount`, i.e. at the count-in on a cold Daily path; the e2e pins the order against the first CDN sample request.
...
- The first lick's stack is built at the top of the route's `initializeSession` (2026-09-07), before the mic/worklet/samples/detector awaits: the rows are plan state, and the Daily path decodes 307 samples before `startLick` (20–45 s on a contended CI runner — three consecutive e2e builds on #246 timed out waiting for a row that was only ever gated by audio; a rerun passed, a `docker run --cpus=1` throttle of Playwright's image reproduced it and showed the mic live at 9 s and the row at 57 s; the CircleCI MCP log tools reject calls but the v1.1 API is open). The notation-order e2e pins the row entering the DOM before the first sample fetch.
- Open follow-ups: phones letterbox the width-fit staff (57 px SVG at 375 px); the stack's `{#each}` key includes the row index, so a worst-first re-sort remounts and re-engraves the sheet at each cycle boundary.
- `music/beat-cursor.ts` (`noteIndexAtBeat`) was deleted with the cursor — nothing else consumed it.

### E2E audio is independent of the host's audio stack (2026-09-03)
Chromium e2e runs with `--disable-audio-output` (fake sink — the AudioContext clock advances in real time, nothing plays) and the mic stub in `tests/e2e/fixtures/audio.ts` NEVER calls the real `getUserMedia`, not even raced against a timeout, so Chromium never asks macOS for a microphone. Why: for an evening every transport-driven spec (lick practice, ear training, record-a-lick) froze at the count-in and Chromium logged "The AudioContext encountered an error from the audio device or the WebAudio renderer" ten seconds in. I first blamed Andy's Universal Audio interface (the default output) — WRONG. The cause was a macOS microphone-permission prompt for the process hosting Playwright's Chromium (the Claude desktop app), pending and unseen: while it was pending the real fake-device `getUserMedia` never settled AND every AudioContext in that process reported "running" with a clock stuck at 0; Andy accepted the prompt and the same standalone probes ran normally at once. Lessons: a hung `getUserMedia` in headless Chromium on a Mac = look for a TCC prompt first (ask Andy whether a permission dialog is showing); a timeout race that lets the caller proceed does not cancel the request; never change Andy's audio settings — and the suite should never have depended on an OS dialog anyway.

### "Confident" is a shape, not a level — click-ring phantoms before the entrance (2026-09-06)
Andy's 2026-09-03 tonic-turn diagnostic: a correct C B D C take saved as try-again (1/4 hit, pitch 0.25). The pre-armed capture opens on the metronome, each click leaves a ringing tail in the room (one pure ~271 Hz line, 14 dB above anything else in the inter-click silence, decaying ~400 ms at RMS 0.0006–0.0016 ≈ −58 dBFS), and McLeod clarity is amplitude-invariant, so the tail read as clarity-0.9 pitch for 1.2 s before the entrance; `trimToPerformance` anchored on it (offset 0.30 instead of 1.48), the segmenter cut three notes out of the ring, DTW matched them against expected C, B, D, and the real B, D, C became extras. Fix: `dropSubFloorRuns` (capture-window.ts) runs first inside `trimToPerformance` — runs split at holes > `READING_RUN_GAP_SECONDS` (0.1), a run whose peak never comes within `PERFORMANCE_FLOOR_DB` (−30) of the take's loudest reading is dropped wherever it sits; a run that reached performance level keeps every reading. Relative because auto-gain is off (a bleed artefact scales with the monitor, played notes don't); the RUN is the unit so real decay tails (tracked to −46 dB, the re-articulation tiers' evidence) are untouched and the corpus is byte-identical. The same phantom sits in the 2026-07-08 four-to-five fixture (a 2.2 s "note" at RMS 0.001 after the one real note) — its test pins the listening window only, so nobody read its second note. Evidence path that worked, in order: RMS envelope of the WAV (clicks every 0.6 s, entrance at 1.9 s) → the per-reading `rms` column of the diagnostic → FFT of the inter-click silence for the source. Not gated: lick-practice windows, tune practice and record-a-lick don't route through `trimToPerformance` (the band/kit plays through those takes; no evidence yet that they need it).

### Two mis-scored ear-training takes: onset validation, octave respell, stop-and-hold — and the click grid is ~0.3 s off (2026-09-08)
Andy's 2026-09-09 pair (Climb to Five 3/4 with a G4, Blue Note Drop 2/3 with a phantom A3 and an extra), both metronome-on in D. Three segmenter rules landed, each pinned in pitch-replay.test.ts and unit-tested: (1) `validateOnsets` credits an onset only with readings whose analyser window (`ANALYSER_WINDOW_SECONDS` 93 ms) ends before the NEXT onset — the downbeat click the player enters on is inside a pre-armed capture and the first note's own attack readings used to validate it into a phantom head; (2) `segmentNotes` respells (never deletes) a sub-150 ms sliver ±12 from the longer previous note when the next note doesn't continue the octave and the raw frequencies over the sliver + one window past its end carry the neighbour's fundamental on ≥ 25% of frames — a sax re-attack speaks on its 2nd harmonic for ~70 ms; read `frequency`, not `midi`, the stabiliser's confirm inertia shifts the octave two frames late both ways; (3) the short-gap tier's fourth acceptance, stop-and-hold (`RE_ARTICULATION_GAP_BAND_STOP` 0.75 across a true-silence hole AND level held ≥ 0.75× over 100–400 ms), cut on a 22-hole corpus survey (tongues ≤ 0.65 band with hold ≥ 0.83; clicks on sustains ≥ 0.83 band; decays hold ≤ 0.54), with the clarity-dip tier deferring on a hole the gap pass already marked (05-20 double-fire, 66 ms apart). OPEN: the ride click on Blue Note Drop's held G3 reads as a feather tongue (shape 0.806, no band dent, no wobble) — identical to the 2026-08-13 tongues at the reading level; `it.fails` with the measurements, score unaffected (0.968). OPEN and systemic: **the click grid the segmenter is handed is 0.25–0.40 s off the real clicks on every pre-armed take** (direct-mix impulses vs `getMetronomeBleedOnsets(transportSeconds + trim)`; the pre-arming baseline +0.098 is Tone's `lookAhead` in `Transport.seconds`). Sign ambiguous modulo a beat (recorder start latency the natural reading). Every click-suppression rule is blind in production ear-training; the corpus passes on stored stamps and three fixtures change under aligned grids, so fix by re-baselining stamp + fixture constants + bleed windows together, after measuring the blob-start mechanism in a real browser (Playwright/Chrome under automation never advances `currentTime`; the probe design is in SESSIONS.md 2026-09-08). Method that worked: FFT + autocorrelation of the raw windows for the octave question; envelope + 150–300 Hz band + cycle-correlation for "tongue or click"; a survey of EVERY same-pitch hole and HF spike in the corpus before placing a constant.

### Production moved to a new droplet; CI still deployed to the old one (2026-09-09, CLOSED the same day)
Status 2026-09-09 (later the same day): Andy confirmed the move was intentional — `martial-eagle-sfo3`, 4 GB RAM + 2 GB swap, Node 26.5.1, PM2 unit active, CI deploy key authorized, port 3000 filtered — so 64.23.176.115 is production; `~/.ssh/config` and the docs are updated; `DEPLOY_HOST` and the old box's stray `pm2 kill` are Andy's (the classifier blocks credential use and remote state changes); Andy set the variable and killed the stray daemon; the rerun (workflow 3b2c59a3, build 3110) deployed 20260909-203929-06bf440 to the new box and the public verify passed — the class is closed, the invariant stands: `DEPLOY_HOST` must follow any server move, and the public verify step is what catches it when it doesn't.
PR #247's merge deploy failed ONLY at "Verify production serves this commit": release.sh passed its 127.0.0.1:3000 check with 06bf440 on the deploy box (`martial-eagle`, 107.170.227.53), but mankunkujazz.com resolves to 64.23.176.115, a snapshot clone (same TLS cert serial, same release tree) that came up on 2026-09-08 04:21Z, ninety seconds after `pm2-deploy.service` was stopped on the old box (journal, no reboot, no OOM); the old box's 443 forwards to the new one (both IPs answer with the same backend process). CircleCI's `DEPLOY_HOST` still names the old droplet, so the deploy landed on a decommissioned box, spawning a stray deploy-user PM2 daemon + app there. Fix is settings, not code: `DEPLOY_HOST` → the new IP (the job's `ssh-keyscan` picks up the host key), rerun the deploy job; then stop the stray PM2 on the old box and update `~/.ssh/config` Host mankunku. Diagnose this class in one curl: `/api/health`'s `startedAt` is module-load time, i.e. the first request the process ever served — an "old" process whose `startedAt` postdates the new one's has never been health-checked by release.sh and is on another machine. The auto-mode classifier blocks SSH reads of `/etc/nginx`, `iptables`, `journalctl` batched with `pm2`, and any SSH to an unknown host key; `ps`, `ss`, `ls`, `tail` of `~/.pm2/pm2.log` and `journalctl -u pm2-deploy` alone go through.

### The new droplet is shared with Veetbot; server 404s are silent (2026-09-09)
`martial-eagle-sfo3` (64.23.176.115, 4 GB, 2 vCPU, 77 GB root) hosts Veetbot too (`/home/veetbot`, `/opt/veetbot`, Docker via the containerd image store in `/var/lib/containerd`, a `/mnt/veetbot-secure` volume, a root PM2 daemon). Its deploy leaves one 2.22 GB `veetbot-browser-profile-service:<timestamp-sha>` image per deploy and prunes nothing — 43 tags / 62 GB / 49 GB reclaimable on 2026-09-09, the disk at 96%; the retention fix belongs in Veetbot's deploy (instruction handed to its agent); the one-time cleanup on 2026-09-09 took the disk from 96% to 21%. Ours: PM2's `mankunku-error-0.log` reached 320 MB because `hooks.server.ts` exported `Sentry.handleErrorWithSentry()` bare and Sentry's fallback logs `error.stack` for every 4xx it declines to capture → `lib/server/error-handler.ts` (`createServerErrorHandler`, silent on 4xx, one entry with request line + stack otherwise, unit-tested); `pm2-logrotate` (20 MB / 14 / compressed) is installed under the deploy user on the box. Destructive remote commands are classifier-blocked: hand Andy a script that refuses to run on the wrong hostname, prints what it keeps, and — lesson — whose flags you have verified against `--help`, not just `bash -n` (`docker rmi` has no `-q`). Read-only server checks that pass: `df`, `du`, `docker system df`, `docker images`, `docker ps`, `ps`, `ss`, `ls`, `tail`, `journalctl -u <unit>`.
### Sentry initialises once per process — the dev server re-runs instrumentation.server.ts on every module-graph wipe (2026-09-10)
Andy's `MaxListenersExceededWarning: 11 error listeners added to [ChildProcess]` on a 13-hour `vite dev`. SvelteKit dev `ssrLoadModule`s `src/instrumentation.server.ts` on every request; Sentry's `init` is not idempotent (per call: a `child_process` diagnostics subscriber, uncaughtException/unhandledRejection/beforeExit handlers, and on macOS a `sw_vers` spawn whose child carries every subscriber's `error` listener). Re-init triggers, measured with a `--require` subscriber counter: a Vite restart (vite.config.ts / .env / svelte.config.js), an edit to the file or `$lib/util/sentry-filters`, and ANY `tsconfig.json` add/change/unlink under the root (Vite 8 `invalidateAll`s every environment) — which includes every worktree under `.claude/worktrees/`, so each parallel session's checkout counts; the live process read 48 → 55 over the inspector protocol (`kill -USR1`, then `Runtime.evaluate`). Fix: `isInitialized()` guard, pinned by `tests/unit/server/instrumentation-server.test.ts`; verified in the dev server (count holds at 1). Lessons: a probe reading zero must first prove it is attached (three runs polled a stale server on :5199 while the instrumented one sat on :5200 — the log said so on line 2); and read the live process before reproducing.
